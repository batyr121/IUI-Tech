...
export const isCreativeTask = (task?: Partial<Task> | null) =>
  /пазл|фрагмент|құрастыр|бөлік|сбор|соедин|сәйкест|пары|жұп|код|code|алгоритм|робот|микро-проект|профориентация|конструктор|истори|story|маршрут|матрица|сканирование|кеңістіктік|пространственная|координац|баланс|хлоп|шапалақ|айқас|память|жады|есте сақ|запомни|карточ|n-back|реті|порядок|дыбыс|звук/i.test(textOf(task));
...
const modeOf = (task: Task) => {
  ...
  if (/память|жады|есте сақ|запомни|карточ|n-back|дыбыс|звук/i.test(text)) return 'match';
