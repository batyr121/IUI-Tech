...
function Auth({onSuccess,onBack}:{onSuccess:()=>void,onBack:()=>void}) {
  ...
  useEffect(()=>{const input=document.querySelector<HTMLInputElement>('.auth-card input[name="password"]');if(input){input.minLength=mode==='signup'?10:1;input.placeholder=mode==='signup'?'Не менее 10 символов':'Введите пароль'}},[mode,role]);
